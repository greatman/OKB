...
public class OKFunctions {
	...
	public static String getEncPass(String user, String pass) {
		...
		if (forumtype.equalsIgnoreCase("bbpress") || forumtype.equals("phpbb") || forumtype.equals("custom") || forumtype.equals("vanilla")) {
			...
			try {
				ResultSet rs = OKDatabase.dbm.query("SELECT data FROM " + (String) getConfig("modes.table2") + "," + (String) getConfig("modes.table1") + " WHERE xf_user.username = '" + user
	...
	public static Integer GetMySQLPosts(Player player) {
		...
		if (identifier != null) {
			try {
				ResultSet check = OKDatabase.dbm.query("SELECT " + (String) OKFunctions.getConfig("posts.field") + " FROM " + (String) OKFunctions.getConfig("modes.table1") + " WHERE " + (String) OKFunctions.getConfig("modes.field1") + " = '"
						+ identifier + "'");
				if (check.next()) {
					do {
